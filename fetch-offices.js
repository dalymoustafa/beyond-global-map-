...
function extractCity(address) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (/^\d+$/.test(part)) continue;
    if (/\b(street|road|avenue|boulevard|floor|level|suite|tower|plaza|bldg|place|utca|district|avenida|rua|heights|zayed|hotel|mayfair|central|kolonaki|georgia|atasehir|blok|kat)\b/i.test(part)) continue;
...
async function main() {
  ...
  const record = allRecords.find(r =>
    r.fields["Name"] && r.fields["Name"].toLowerCase().includes("beyond global")
  );

  if (!record) { console.error("Could not find Beyond Global Partners!"); process.exit(1); }
...
<head>
  ...
  <title>Beyond Global Partners — Office Locations</title>
...
<script>
  ...
  fetch("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson")
    ...
    .then(geojson => {
      ...
      if (OFFICES.length > 0) { map.fitBounds(OFFICES.map(o => [o.lat, o.lng]), { padding: [30, 30], maxZoom: 4 }); } else { map.setView([20, 10], 2); }
    });
  map.setView([20, 10], 2);
<\/script>
</body>
